...
function find_pattern_start()
    ...
    for i = 1, 1000000000 do
        ...
        local value = calculate_next_score()

        if prev_score == value then
            return i, prev_score
        else
            prev_score = value
        end
...
local idx, score = find_pattern_start()
print( idx, score )
...
local i = 1
while next ~= score do
    next = calculate_next_score()
    pattern[ i ] = next
    i = i + 1
end

function get_index( i, offset, size )
    local idx = ( i - offset ) % size
    if idx == 0 then idx = size end
    return idx
end

local idx = get_index( 1000000000, 433, i )
print ( pattern[ idx ] )